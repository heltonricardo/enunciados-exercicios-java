...
Tags: `api` `cliente-servidor` `crud` `documentação` `hibernate` `jpa` `mysql` `open-api` `relacionamento` `rest` `spring initializr` `spring boot` `swagger` `teste automatizado` `teste unitário/de unidade` `validation`
